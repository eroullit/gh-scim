...
7. **Retries:** `gh-scim` exposes an error's HTTP status and parsed message, but not response headers. Retry `429` and transient `5xx` responses with bounded exponential backoff and jitter. Retry a `403` only when its message clearly identifies rate limiting; otherwise require correction of the underlying data or configuration before retrying `400`, `401`, `403`, or `409` responses. If exact `Retry-After` or rate-limit reset timing is required, first extend the CLI to expose those headers in machine-readable form.
